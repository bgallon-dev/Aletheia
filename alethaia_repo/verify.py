...
class VerificationResult:
    ...
    def format_report(self, verbose: bool = True) -> str:
        ...
        if not self.forensic_match and not self.forensic_skipped and self.localization:
            lines.append("\n[Coarse Localization]")
            lines.append(f"  Detected {len(self.localization)} modified region(s) at baseline resolution:")
            
            for i, (start_win, end_win, start_byte, end_byte) in enumerate(self.localization, 1):
                window_count = end_win - start_win + 1
                byte_count = end_byte - start_byte
                
                lines.append(f"\n  Region {i}:")
                lines.append(f"    Windows:  {start_win} - {end_win} ({window_count} windows)")
                lines.append(f"    Bytes:    {start_byte} - {end_byte} ({self._format_bytes(byte_count)})")
        
        # Zoom scan results
        if self.zoom_performed and self.zoom_regions:
            lines.append("\n[Zoom Scan - High Resolution Localization]")
            lines.append(f"  Resolution: WS={ZOOM_WINDOW_SIZE} bytes ({ZOOM_WINDOW_SIZE // 1024} KiB), "
            ...
            for i, zoom_region in enumerate(self.zoom_regions, 1):
                lines.append(f"  Zoom Region {i} (from coarse windows {zoom_region.coarse_start_window}-{zoom_region.coarse_end_window}):")
                lines.append(f"    Scan range: bytes {zoom_region.zoom_start_byte} - {zoom_region.zoom_end_byte}")
                
                if zoom_region.fine_regions:
                    lines.append(f"    Found {len(zoom_region.fine_regions)} fine-grained difference(s):")
                    for j, (start_win, end_win, start_byte, end_byte) in enumerate(zoom_region.fine_regions, 1):
                        window_count = end_win - start_win + 1
                        byte_count = end_byte - start_byte
                        ...
                        lines.append(f"\n      Difference {j}:")
                        lines.append(f"        Windows:  {start_win} - {end_win} ({window_count} windows @ zoom resolution)")
                        lines.append(f"        Bytes:    {abs_start_byte} - {abs_end_byte} ({self._format_bytes(byte_count)})")
                else:
                    lines.append(f"    No fine-grained differences (coarse mismatch may be quantization artifact)")
...
class ArtifactVerifier:
    ...
    def verify(
        ...
        result.cryptographic_match = (result.content_hash_actual == result.content_hash_expected)
        ...
        try:
            ...
            if file_size < window_size:
                ...
                if verbose:
                    ...
                    print(f"    File size: {file_size} bytes ({self._format_file_size(file_size)})")
                    print(f"    Window:    {window_size} bytes ({self._format_file_size(window_size)})")
                    print(f"    Scanner will compute single window covering entire file")
            ...
            try:
                ...
                result.forensic_skip_reason = f"Barcode computation failed: {scan_error}"
            ...
            result.forensic_match = (result.barcode_hash_actual == result.barcode_hash_expected)
                ...
                if stored_albc:
                    ...
                    if stored_parsed and actual_parsed:
                        ...
                        if verbose and result.localization:
                            print(f"  Found {len(result.localization)} modified region(s)")
    ...
    def _perform_zoom_scan(
        ...
        try:
            ...
            for coarse_start_win, coarse_end_win, coarse_start_byte, coarse_end_byte in result.localization:
                ...
                if verbose:
                    print(f"  Zooming into region: bytes {zoom_start:,}-{zoom_end:,} "
                        ...
                        for win_idx in range(num_windows):
                            if baseline_bc[win_idx] != suspect_bc[win_idx]:
                                ...
                                if in_diff:
                                    ...
                                    end_byte = zoom_start + win_idx * ZOOM_STEP_SIZE + ZOOM_WINDOW_SIZE
                                    fine_regions.append((diff_start_win, win_idx, start_byte, end_byte))
                                    in_diff = False
                        
                        # Handle difference extending to end
                        if in_diff:
                            start_byte = zoom_start + diff_start_win * ZOOM_STEP_SIZE
                            end_byte = zoom_start + num_windows * ZOOM_STEP_SIZE + ZOOM_WINDOW_SIZE
                            fine_regions.append((diff_start_win, num_windows, start_byte, end_byte))
                        
                        zoom_region.fine_regions = fine_regions
                        
                        if verbose:
                            if fine_regions:
                                print(f"    Found {len(fine_regions)} fine-grained difference(s)")
                                for start_win, end_win, start_byte, end_byte in fine_regions[:3]:
                                    print(f"      Windows {start_win}-{end_win}: bytes {start_byte:,}-{end_byte:,}")
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python verify.py <artifact_id> --file <path> [options]", file=sys.stderr)
        print("\nOptions:", file=sys.stderr)
        print("  --repo <path>    Repository root (default: .)", file=sys.stderr)
        print("  --quiet          Suppress verbose output", file=sys.stderr)
        print("  --no-zoom        Disable zoom scan (only coarse localization)", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        result = verifier.verify(artifact_id, file_path, verbose=verbose, enable_zoom=enable_zoom)
